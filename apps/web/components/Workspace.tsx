        ...
        <div className="w-full max-w-3xl flex flex-col items-center relative z-20 animate-in fade-in slide-in-from-bottom-8 duration-1000">
            ...
            <div className="flex w-full mb-4 gap-2">
                {(["record", "paste", "upload"] as const).map((tab) => (
                    <button
                        key={tab}
                        onClick={() => status !== "processing" && setMode(tab)}
                        disabled={status === "processing" || (!canInteract && tab !== "paste")}
                        className={`
            ...
            <div className="w-full bg-[#121214] border border-white/5 rounded-b-xl rounded-t-sm shadow-[0_20px_50px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.05)] relative overflow-hidden flex flex-col min-h-[500px]">
                ...
                {/* TEXT INPUT AREA OR COMPLETED STATE */}
                <div className="flex-1 w-full relative z-10 p-8 flex flex-col">
                    {status === "completed" ? (
                        <div className="absolute inset-0 flex flex-col items-center justify-center animate-in fade-in zoom-in-95 duration-700">
                            <div className="w-16 h-16 bg-green-500/10 rounded-full flex items-center justify-center mb-6 shadow-[0_0_30px_rgba(34,197,94,0.2)]">
                                <svg className="w-8 h-8 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                                </svg>
                            </div>
                            <span className="text-[20px] font-light tracking-widest text-[#F5F5F5] mb-12">
                                Exported to Notion
                            </span>
                            ...
                            <div className="flex items-center gap-4">
                                {result?.notion_url && (
                                    <a
                                        ...
                                        className="px-8 py-4 border border-white/10 hover:border-white/20 hover:bg-white/[0.02] text-[12px] font-bold tracking-[0.2em] uppercase text-zinc-300 transition-all active:scale-95 rounded-full"
                                    >
                                        Open in Notion
                                    </a>
                                )}
                                <button
                                    ...
                                    className="px-8 py-4 bg-indigo-600 hover:bg-indigo-500 text-white text-[12px] font-bold tracking-[0.2em] uppercase transition-all shadow-[0_4px_20px_rgba(99,102,241,0.3)] hover:shadow-[0_4px_25px_rgba(99,102,241,0.5)] transform hover:-translate-y-0.5 active:scale-95 rounded-full"
                                >
                        ...
                        <>
                            <textarea
                                ...
                                disabled={status === "processing" || mode === "record" || !canInteract}
                                className={`
                                    ...
                                    ${status === "processing" ? 'opacity-10' : 'opacity-100'}
                                `}
                            />
                ...
                {(error || micError === "denied") && status !== "processing" && (
                    <div className="absolute inset-x-0 top-0 bg-red-500/10 border-b border-red-500/20 px-8 py-4 flex justify-between items-center z-50">
                        ...
                        <button onClick={() => { setError(null); setMicError(null); if (status === "error") setStatus("idle"); }} className="text-xs text-zinc-400 hover:text-white uppercase tracking-widest">Dismiss</button>
                    </div>
                )}

                {/* COMMAND TOOLBAR (Bottom anchored) */}
                {status !== "completed" && (
                    <div className="w-full border-t border-white/5 bg-[#0B0D10]/40 p-6 flex justify-between items-center relative z-10 transition-opacity duration-500">
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            <div className={`relative flex items-center justify-center transition-all ${mode === "record" ? 'scale-110' : 'opacity-60 hover:opacity-100'}`}>
                                ...
                                <div className="scale-125 z-10 text-white">
                                    <MicButton
                                        ...
                                        disabled={!canInteract || status === "processing" || mode !== "record"}
                            ...
                            <div className={`transition-all hover:scale-110 text-white ${mode === "upload" ? 'opacity-100' : 'opacity-60'}`}>
                                <div className="scale-125">
                                    <FileUploadButton onFileContent={handleFileContent} disabled={!canInteract || status === "processing" || mode !== "upload"} />
                                </div>
                            </div>
                        ...
                        <button
                            ...
                            disabled={!canInteract || status === "processing" || transcript.trim().length === 0 || limitExceeded}
                            className={`
                                px-8 py-3 rounded-full text-[13px] font-bold tracking-widest uppercase transition-all duration-300 flex items-center gap-3
                                ${transcript.trim().length > 0 && status !== "processing" && !limitExceeded
