    ...
    try {
        ...
        if (!response.ok) {
            return reply
                ...
                .send({ error: data.message || "Failed to exchange code" });
        }
...
app.post("/runs/:id/export-notion", async (request, reply) => {
    ...
    if (!session) {
        return reply
            .status(401)
            .send({ error: "Unauthorized. Please connect Notion." });
    }
...
app.post("/runs/:id/ingest", async (request, reply) => {
    ...
    if (contentType.includes("multipart/form-data")) {
        return reply.status(400).send({
            error:
                "File uploads are not supported. Send JSON with { transcript: string }.",
        });
    }
...
app.post("/wispr/webhook", async (request, reply) => {
    ...
    if (wordCount > 10_000) {
        request.log.info(
            { event: "transcript_blocked_large", word_count: wordCount },
            "wispr/webhook: transcript too large",
        );
        return reply.status(400).send({
            error: "transcript_too_large",
            message:
                "Transcript exceeds 10,000 words. Please split into two sessions for best fidelity.",
    ...
    if (existing) {
        request.log.info(
            ...
            "wispr/webhook: duplicate transcript",
        );
        return {
            run_id: existing.id,
    ...
    if (createError || !created) {
        ...
        return reply
            .status(500)
            .send({ error: createError?.message ?? "Failed to create run." });
    }

    const runId: string = created.id;
    request.log.info(
        { run_id: runId, transcript_hash: transcriptHash, resolution: "new_run" },
        "wispr/webhook: run created",
    );
    ...
    try {
        ...
        const retentionPct = +(finalCleanedCharCount / originalCharCount).toFixed(
            3,
        );
        ...
        try {
            ...
            if (themes && Array.isArray((themes as any).themes)) {
                for (const t of (themes as any).themes) {
                    ...
                    genericPhrases.forEach((gp) => {
                        if (summary.includes(gp)) detectedGeneric.push(gp);
                    });
                }
            }
            request.log.info(
                {
                    step: "telemetry_theme",
                    quote_verbatim_match: quoteMatch,
                    detected_generic_phrases: [...new Set(detectedGeneric)],
                },
                "wispr/webhook: theme telemetry",
            );
        } catch (e) { }
        // -------------------------
        ...
        try {
            ...
            if (parsedAngles && Array.isArray(parsedAngles.angles)) {
                for (const a of parsedAngles.angles) {
                    ...
                    const expansion = (
                        (a as any).expansion_direction || ""
                    ).toLowerCase();
                    genericPhrases.forEach((gp) => {
                        if (expansion.includes(gp)) detectedGeneric.push(gp);
                    });
                }
            }
            request.log.info(
                {
                    step: "telemetry_angles",
                    hook_verbatim_match: hookMatch,
                    detected_generic_phrases: [...new Set(detectedGeneric)],
                },
                "wispr/webhook: angles telemetry",
            );
        } catch (e) { }
        // -------------------------
        ...
        const message = err instanceof Error ? err.message : String(err);
        request.log.error(
            { run_id: runId, error_step: currentStep, error_message: message },
            "wispr/webhook: pipeline failed",
        );
        ...
        if (currentStep === "export-notion") {
            ...
            return reply.status(500).send({
                ...
                failure_step: "export-notion",
                failure_location:
                    err.failure_location || (isNotionTimeout ? "timeout" : "unknown"),
                total_pipeline_time_ms: stepTimings["total"] || 0,
...
app.get("/wispr/health", async (_request, reply) => {
    const checks: { db: boolean; llm: boolean; errors: string[] } = {
        db: false,
        llm: false,
        errors: [],
    };
